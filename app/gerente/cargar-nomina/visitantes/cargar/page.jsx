...
export default function CargarVisitantes() {
  ...
  function handleFile(e) {
    ...
    if (!validarNombreArchivo(file.name, "visitantes")) {
      setErrorFormato(`❌ El archivo '${file.name}' no corresponde a la categoría de Visitantes.`);
      if (fileInputRef.current) fileInputRef.current.value = "";
      return;
    }
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < dataRows.length; i++) {
          ...
          // Read Ficha if present, otherwise fallback to Cedula
          let ficha = "";
          if (fichaIdx !== -1 && getValue(fichaIdx)) {
            ficha = getValue(fichaIdx);
          } else {
            ficha = formattedCedula || cleanCed;
          }
        ...
        const dbConflicts = await checkDuplicateFichasInDB(Array.from(internalFichas), "visitantes");
  ...
  async function guardarNomina() {
    ...
    const ok = confirm("⚠️ ¿Está seguro de que desea reemplazar toda la nómina actual de Visitantes? Esta acción no se puede deshacer y borrará los registros anteriores.");
    if (!ok) return;

    try {

      await setDoc(doc(db, "nominas", "visitantes"), {
        datos: data
      });
      ...
      await registrarAuditoria(
        ...
        `Se importo y reemplazo la nomina de Visitantes vía Excel (total registros: ${data.length}).`
      );

      alert("✅ Nómina de Visitantes guardada");
  ...
  return (
    <div className="main">
      ...
      <div className="panelTitle">
        <h1>Cargar Nómina - Visitantes</h1>
      </div>
        ...
        {data.length > 0 && (
          <>
            <p className="contador">
              Total de visitantes: {data.length}
            </p>
